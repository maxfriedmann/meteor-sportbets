app.controller("CompetitionController", [ "$scope", "$routeParams", "autorun", "Restangular", "$modal", "$location", function($scope, $routeParams, autorun, Restangular, $modal, $location)
{
	...
	{
		Meteor.call("updateCompetition", competition._id, function(error, result)
		{
			if (error)
				alert(error);
		});
	...
	{
		Meteor.call("updatePoints", competition._id, function(error, result)
		{
			if (error)
				alert(error);
		});
	...
	$scope.loadMatches = function(start, limit)
	{
		...
		if ($scope.competition)
		{
			...
			else
			{
				$scope.matches = Matches.find({
					"competitionId" : $scope.competition._id,
					"group_id" : String($scope.selectedGroup.group_id)
				}, {
					sort : {
						match_date_time_utc : 1
					},
	...
	$scope.placeDeltaBet = function(match, homeDelta, awayDelta)
	{
		...
		$scope.progressComponent("bet-area-" + match.match_id, true);
		
		BetService.placeDeltaBet(parseInt(match.match_id), match.competitionId, parseInt(homeDelta), parseInt(awayDelta), function(error, result)
		{
			if (error)
			{
			...
			$scope.progressComponent("bet-area-" + match.match_id, false);
		});
	};
